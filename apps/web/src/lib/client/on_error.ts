...
function GetErrorMessage(error: FieldError | undefined) {
	...
	const arrayError = (error as unknown as Record<string, Record<string, FieldError>>)[
...
export function OnError(errors: FieldErrors) {
	const err = GetErrorMessage(errors[Object.keys(errors)[0] ?? "invalid"] as FieldError);
	if (!err) return;
	toast.error(err);
}
